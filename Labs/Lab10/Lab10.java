...
public class Lab10 {
	...
	private class ControlListener implements ActionListener {
		public void actionPerformed(ActionEvent e) {
			...
			if (listMovies == e.getSource()) {
				info.setText(movies.toString());
				theWindow.pack();
			} else if(quit == e.getSource()){
				int selection = JOptionPane.showConfirmDialog(controlPanel, "Do you want to quit?");
				if(selection ==0)
					System.exit(0);
			} else if(addMovie == e.getSource()){
				String movieName = JOptionPane.showInputDialog(theWindow, "What's the name of the movie?");
				String directorName = JOptionPane.showInputDialog(theWindow, "What's the name of the Director?");
				String studioName = JOptionPane.showInputDialog(theWindow, "What's the name of the studio?");
				String grossVal = JOptionPane.showInputDialog(theWindow, "Enter the gross value of the movie");
				Double g = Double.parseDouble(grossVal);
				Movie m = new Movie(movieName, directorName, studioName, g);
				movies.addMovie(m);
			} else if(findMovie == e.getSource()){
				String findMyMovie = JOptionPane.showInputDialog(theWindow, "What's the name of the movie?");
				Movie movieFound = movies.findMovie(findMyMovie);
				if(movieFound!=null){
				info.setText(movieFound.toString());
				theWindow.pack();
				} else{
					info.setText(findMyMovie+" does not exist in the file.");
				}
			}
		}
	}
